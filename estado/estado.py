...
async def upload_file(request: Request, file: UploadFile = File(...)):
    ...
    try:
        ...
        if "Estado Reportes" not in excel_file.sheet_names:
            request.session["messages"] = ["El archivo Excel no contiene la hoja 'Estado Reportes'"]
            return RedirectResponse(url="/", status_code=status.HTTP_302_FOUND)
...
def process_preview_data(df):
    ...
    required_columns = {
        ...
        'titulo': next((col for col in df.columns if 'Titulo' in col or 'Título' in col), None),
...
def process_email_data(df):
    """Procesa el DataFrame para generar los datos agrupados por Data Owner y dominio."""
    ...
    if not all([data_owner_col, dominio_col, endorsement_col, visible_col]):
    ...
    for data_owner in df[data_owner_col].unique():
        ...
        for dominio in df_owner[dominio_col].unique():
            ...
            # Conteos según el estado de Endorsement
            total_reportes = len(df_domain)
            
            # Pendientes de certificar: Sin Endorsement (en blanco o nan)
            pendientes_certificar = len(df_domain[
                (df_domain[endorsement_col].isna()) | 
                (df_domain[endorsement_col].str.strip() == '') |
                (df_domain[endorsement_col] == 'nan')
            ])
            
            # Pendientes de publicar: Certificados pero no visibles
            pendientes_publicar = len(df_domain[
                (df_domain[endorsement_col].str.contains('certified', na=False)) & 
                (df_domain[visible_col] == False)
            ])
            
            # Pendientes de promocionar: Certificados y visibles
            pendientes_promocionar = len(df_domain[
                (df_domain[endorsement_col].str.contains('certified', na=False)) & 
                ~(df_domain[endorsement_col].str.contains('promoted', na=False))
            ])
            
            print(f"""
            Debug para {data_owner} - {dominio}:
            - Total reportes: {total_reportes}
            - Sin certificación: {pendientes_certificar}
            - Certificados no visibles: {pendientes_publicar}
            - Pendientes promoción: {pendientes_promocionar}
            - Estados únicos: {df_domain[endorsement_col].unique()}
            """)
            
            tabla_filas = f"""
            <tr>
                <td>{total_reportes}</td>
                <td>{pendientes_certificar}</td>
                <td>{pendientes_publicar}</td>
                <td>{pendientes_promocionar}</td>
            </tr>
            """
...
async def send_emails(request: Request):
    ...
    try:
        ...
        try:
            ...
            request.session["messages"] = ["Error de autenticación. Verifique las credenciales."]
            return RedirectResponse(url="/confirm_send", status_code=status.HTTP_302_FOUND)
        except Exception as smtp_error:
            print(f"Error de conexión SMTP: {str(smtp_error)}")
            request.session["messages"] = ["Error al conectar con el servidor de correo."]
            return RedirectResponse(url="/confirm_send", status_code=status.HTTP_302_FOUND)
        ...
        history = {
            ...
            "total_domains": len(set(domain for email in sent_emails for domain in email["domains"])),
